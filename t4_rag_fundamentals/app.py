...
def main(rag: _MicrowaveRAG):
    ...
    while True:
        ...
        #   - Step 1 (Retrieval): call `rag.retrieve_context` with `user_question` to fetch relevant chunks and save it as `context`
        #   - Step 2 (Augmentation): call `rag.augment_prompt` with `user_question` and `context` to build the prompt and save it as `augmented_prompt`
        #   - Step 3 (Generation):  call `rag.generate_answer` with `augmented_prompt` to get the LLM answer
