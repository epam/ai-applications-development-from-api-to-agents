...
class UMSDataGuardrail:
    ...
    def _build_analyzer() -> AnalyzerEngine:
        #TODO:
        # 1. Create `NlpEngineProvider(nlp_configuration=_NLP_CONFIG)` and call `.create_engine()` on it
        # 2. Create `AnalyzerEngine(nlp_engine=...)` with the engine from step 1
        # 3. Register `_UMSCreditCardRecognizer()` via `analyzer.registry.add_recognizer(...)`
        # 4. Register `_UMSSalaryRecognizer()` the same way
        # 5. Return the configured analyzer
        raise NotImplementedError()

    def redact(self, text: str) -> str:
        """Redact credit card and salary data via Presidio analyze → anonymize pipeline."""
        #TODO:
        # 1. Call `self.analyzer.analyze(text=text, language="en", entities=self._ENTITIES)` and store results
        # 2. If results is empty, return `text` unchanged
        # 3. Call `self.anonymizer.anonymize(text=text, analyzer_results=results, operators=self._OPERATORS)`
